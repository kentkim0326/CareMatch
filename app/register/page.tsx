...
const T = {
  ko: {
    ...
    region: '활동 지역',
    regions: ['서울', '경기', '부산', '대구', '인천', '광주', '대전', '울산', '제주', '도쿄', '오사카', '교토'],
    wdays: ['월', '화', '수', '목', '금', '토', '일'],
    timeSlots: ['06-09시', '09-12시', '12-15시', '15-18시', '18-21시', '야간'],
    skills: ['치매 케어', '거동 보조', '식사 보조', '투약 관리', '재활 보조', '말벗·정서', '가사 지원', '목욕 보조', '병원 동행', '야간 케어', '이중언어', '응급 대응'],
    intro: '자기소개',
    introPh: '어르신을 돌보는 마음가짐이나 경력을 짧게 적어주세요.',
    avatarLabel: '프로필 아이콘 선택',
    doneTitle: '등록이 완료되었습니다!',
    doneDesc: '검토 후 24시간 내 승인 알림을 드립니다.',
    donePoints: '+50 케어포인트 지급 예정',
    goMatch: '매칭 보러 가기 →',
    goHome: '홈으로',
  },
  ja: {
    title: '介護士登録',
    steps: ['基本情報', '活動地域', 'スケジュール', '専門性'],
    back: '← 戻る',
    next: '次へ',
    submit: '登録完了',
    name: 'ニックネーム',
    namePh: 'サービス内で表示される名前',
    age: '年齢層',
    ages: ['20代', '30代', '40代', '50代', '60代+'],
    lang: '話せる言語',
    langs: ['한국어', '日本語', 'English'],
    region: '活動地域',
    regions: ['ソウル', '京畿', '釜山', '大邱', '仁川', '光州', '大田', '蔚山', '済州', '東京', '大阪', '京都'],
    wdays: ['月', '火', '水', '木', '金', '土', '日'],
    timeSlots: ['06-09時', '09-12時', '12-15時', '15-18時', '18-21時', '夜間'],
    skills: ['認知症ケア', '移動介助', '食事介助', '服薬管理', 'リハビリ補助', '傾聴・感情サポート', '家事支援', '入浴介助', '通院同行', '夜間ケア', 'バイリンガル', '緊急対応'],
    intro: '自己紹介',
    introPh: 'ご高齢の方へのケアの姿勢や経歴を簡単にご記入ください。',
    avatarLabel: 'プロフィールアイコン選択',
    doneTitle: '登録が完了しました！',
    doneDesc: '審査後、24時間以内に承認通知をお送りします。',
    donePoints: '+50 ケアポイント付与予定',
    goMatch: 'マッチングを見る →',
    goHome: 'ホームへ',
  },
};
      ...
      <div className={styles.doneWrap}>
        <div className={styles.doneCard}>
          ...
          <div className={styles.btnRow}>
            <a href="/match"><button className={styles.btnPrimary}>{t.goMatch}</button></a>
            <a href="/"><button className={styles.btnOutline}>{t.goHome}</button></a>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div className={`${styles.root} ${lang === 'ja' ? styles.rootJa : ''}`}>
      ...
      <div className={styles.appHdr}>
        {step > 0
          ? <button className={styles.backBtn} onClick={() => setStep(s => (s - 1) as Step)}>{t.back}</button>
          : <a href="/"><button className={styles.backBtn}>{t.back}</button></a>
        }
        <span className={styles.appTitle}>{t.title} — {t.steps[step]}</span>
      ...
      <div className={styles.formWrap}>
        ...
        {step === 0 && (
          <>
            <div className={styles.fCard}>
              <p className={styles.fTitle}>{t.avatarLabel}</p>
            ...
            <div className={styles.fCard}>
              <div className={styles.fRow}>
                <label>{t.name}</label>
                <input value={nick} onChange={e=>setNick(e.target.value)} placeholder={t.namePh} maxLength={12} />
              </div>
              <div className={styles.fRow}>
                <label>{t.age}</label>
        ...
        {step === 1 && (
          <div className={styles.fCard}>
            <p className={styles.fTitle}>{t.region}</p>
            <p className={styles.fDesc}>{lang==='ko'?'활동 가능한 지역을 모두 선택하세요 (복수 선택 가능)':'活動可能な地域をすべて選択してください（複数選択可）'}</p>
            ...
            {regions.length > 0 && (
              <div className={styles.infoBox}>
                {lang==='ko'?`선택된 지역: ${regions.join(', ')}`:`選択された地域: ${regions.join(', ')}`}
              </div>
            )}
          </div>
        )}
        ...
        {step === 3 && (
          <>
            <div className={styles.fCard}>
              ...
              <p className={styles.fDesc}>{lang==='ko'?'해당하는 항목을 모두 선택하세요':'該当する項目をすべて選択してください'}</p>
              <div className={styles.filterRow} style={{flexWrap:'wrap'}}>
                {t.skills.map(s => (
            ...
            <div className={styles.fCard}>
              <div className={styles.fRow}>
                ...
                <textarea value={intro} onChange={e=>setIntro(e.target.value)}
                  placeholder={t.introPh} rows={4} maxLength={200} />
              </div>
